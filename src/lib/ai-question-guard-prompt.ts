...
ユーザーは音楽チャット部屋で「@」付きで AI に質問します。

【判定方針（最優先）】
- **疑わしい場合は musicRelated true**（明らかに音楽無関係と断定できるときだけ false）。
- 直近ログに選曲・曲解説・YouTube・アーティスト名・楽曲の話があれば、短い確認・感想・代名詞だけでも **true**。
- false にしてよいのは、天気・株・政治・プログラミング・宿題・一般医学相談など、**音楽チャットとしてほぼ確実に無関係**なものだけ。

次の質問文が、次のいずれかに当てはまる場合は musicRelated を true にしてください。
...
- **特定のミュージシャン・アーティスト名があり、楽曲への参加・共演・featuring・プロデュース・ゲスト出演**を述べる・確認する短い発言（例：「スティービーワンダーが参加してますね」「〇〇がプロデュース？」「Stevie Wonder produced this?」）は **musicRelated true**（直前に選曲・曲解説があれば特に true。曲名が無くても true）

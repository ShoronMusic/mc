...
export async function generateChatReply(
  ...
  if (currentSong && currentSong.trim()) {
    ...
    songContext +=
      '・曲やアーティストについての質問には、知っている範囲で事実に基づいて簡潔に答えてください。リリース年・アルバム・背景のほか、当該アーティストの現在の活動状況（解散・休止・ソロ活動・再結成など）を聞かれた場合も、事実に基づいて答えてください。\n';
    songContext +=
...
・曲の事実を聞かれたら（何年リリース？など）、知っていればそのまま簡潔に答える（例：「1994年です」）。
...
上記への返事だけを出力してください。挨拶や説明は不要。`;
  ...
  try {
    const isRejectedByPolicy = (txt: string | null | undefined): boolean => {
      const t = (txt ?? '').trim();
      if (!t) return false;
      const hasEvidence = /出典|ソース|Wikipedia|公式|根拠/i.test(t);
      const r = [
      ...
      return !hasEvidence && r.some((x) => x.test(t));
    };
    ...
    while (attempt < 2) {
      ...
      if (text && !isRejectedByPolicy(text)) return text;
      if (attempt >= 2) return text || null;
      // 2回目：断定しない/根拠なしバズ/チャート/受賞/制作期間/録音工程を避ける方向へ寄せる
      prompt2 = prompt + '（追加指示）根拠がない断定を避け、歌詞テーマの要点とサウンドの特徴（印象）だけで短く書いてください。';
    }
    return null;
  } catch (e) {
...
${mainArtistInstruction}${generalInstruction}・「今流れている」を繰り返さない。
・押し付けがましくなく、さりげなく。日本語で、です・ます調。40文字以上100文字以内。本文だけ出力。`;
  ...
  try {
    ...
    while (attempt < 2) {
      ...
      if (text && !isRejectedByPolicy(text)) return text;
      if (attempt >= 2) return text || null;
      // 2回目：断定・根拠なしバズ/チャート/受賞/制作期間/録音工程を避ける
      prompt2 =
        prompt +
        '\n（追加指示）根拠がない「ブーム/拡散/チャート上位/受賞/制作期間/録音工程」の断定を避け、歌詞テーマの要点とサウンドの特徴（印象）だけで短く書いてください。';
    }
    return null;
  } catch (e) {
...
・必ずアーティスト名と曲のリリース情報（リリース年、アルバムやサウンドトラック名など）を入れること。このコメントは「この曲・このアーティストの基本情報」に限定すること。

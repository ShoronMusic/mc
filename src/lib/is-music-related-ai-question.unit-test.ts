...
assert.equal(isMusicRelatedAiQuestion('DragonForceのギターテクについて'), true);
assert.equal(isMusicRelatedAiQuestion('ドラゴンフォースの代表曲は？'), true);
assert.equal(isMusicRelatedAiQuestion('ドラフォって何ジャンル？'), true);
assert.equal(isMusicRelatedAiQuestion('ﾄﾞﾗﾌｫの概要教えて？'), true);
assert.equal(isMusicRelatedAiQuestion('ディランの現在は？'), true);
assert.equal(isMusicRelatedAiQuestion('ホワイトスネイクは今、活動しているの？'), true);
assert.equal(
  isMusicRelatedAiQuestion('ジョン・ボン・ジョヴの喉のリハビリはどうなんですかね？'),
  true
);

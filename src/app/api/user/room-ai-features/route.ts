...
import {
  ...
  USER_ROOM_AI_FEATURES_PERSIST_HINT,
...
function needsUserRoomAiFeaturesSetup(errorMessage: string): boolean {
  if (isMissingTableError(errorMessage)) return true;
  return /ai_next_song_recommend_enabled/i.test(errorMessage) && /column|does not exist/i.test(errorMessage);
}
...
export async function GET() {
  try {
    ...
    if (error) {
      if (needsUserRoomAiFeaturesSetup(error.message)) {
        ...
        return NextResponse.json({
          ...
          persistHint: USER_ROOM_AI_FEATURES_PERSIST_HINT,
        });
      }
...
export async function PUT(request: Request) {
  try {
    ...
    if (error) {
      if (needsUserRoomAiFeaturesSetup(error.message)) {
        return NextResponse.json({ error: USER_ROOM_AI_FEATURES_PERSIST_HINT }, { status: 503 });
      }

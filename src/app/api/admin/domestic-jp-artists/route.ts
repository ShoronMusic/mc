...
import {
  ...
  invalidateDomesticJpArtistCache,
  listDomesticJpArtists,
  normalizeDomesticJpArtistKey,
  validateDomesticJpArtistNameInput,
  validateDomesticJpArtistNoteInput,
...
const TABLE_HINT =
  'docs/supabase-setup.md の「24.1 邦楽扱い英字アーティスト」SQL を Supabase SQL Editor で実行してください。';

function tableMissingResponse() {
  return NextResponse.json(
    {
      error: 'domestic_jp_artists テーブルがありません。',
      hint: TABLE_HINT,
      rows: [],
    },
    { status: 503 },
...
export async function GET() {
  ...
  try {
    const { rows, tableMissing } = await listDomesticJpArtists(admin);
...
export async function POST(request: Request) {
  ...
  const artistName = validateDomesticJpArtistNameInput(body.artist_name);
  if (!artistName) {
    return NextResponse.json(
  ...
  const note = validateDomesticJpArtistNoteInput(body.note);
  ...
  const nameKey = normalizeDomesticJpArtistKey(artistName);
  ...
  await ensureDomesticJpArtistCache(admin);
...
export async function PATCH(request: Request) {
  ...
  if (body.artist_name !== undefined) {
    ...
    patch.name_key = normalizeDomesticJpArtistKey(artistName);
  }

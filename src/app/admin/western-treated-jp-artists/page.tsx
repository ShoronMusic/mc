...
export default function AdminWesternTreatedJpArtistsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setHint(null);
    try {
      const res = await fetch('/api/admin/western-treated-jp-artists', { credentials: 'include' });
  ...
  const createRow = useCallback(async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch('/api/admin/western-treated-jp-artists', {
        method: 'POST',
        credentials: 'include',
  ...
  const saveEdit = useCallback(async () => {
    if (!editingId) return;
    const artistName = editName.trim();
    if (!artistName) {
      alert('アーティスト名を入力してください。');
      return;
    }
  ...
  const deleteRow = useCallback(
    async (id: string, artistName: string) => {
      if (!window.confirm(`「${artistName}」を削除しますか？`)) return;
      setDeletingId(id);
      try {
        const res = await fetch(
          `/api/admin/western-treated-jp-artists?id=${encodeURIComponent(id)}`,
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl px-4 py-8 text-gray-100 sm:px-6">
      <AdminMenuBar />
      <h1 className="mb-2 text-xl font-semibold text-white">洋楽扱い日本人アーティスト</h1>
      <p className="mb-6 max-w-3xl text-sm text-gray-400">
        邦楽判定から除外し、洋楽と同様に扱う日本人アーティスト名を登録します。AI 曲解説の邦楽節約・管理ライブラリの邦楽除外・選曲紹介の「（邦楽）」タグ・
        <code className="rounded bg-gray-800 px-1">catalog_scope</code> 推定に反映されます。
        部屋ライブラリでは洋楽・邦楽どちらのタブからも検索できます（邦楽登録で日本語表記の曲はチャンネル英字 slug でも照合）。
        照合は小文字化＋空白除去後の完全一致です（例: ONE OK ROCK → oneokrock）。
      </p>
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-400">登録がありません。</p>
      ) : (
                  ...
                  <tr key={r.id} className="border-b border-gray-800/80 align-top">
                    <td className="px-3 py-2">
                      {isEditing ? (
                        ...
                        <span className="font-medium text-gray-100">{r.artist_name}</span>
                      )}
                    </td>
                    <td className="px-3 py-2 font-mono text-xs text-gray-500">
                      {isEditing ? '保存後に更新' : r.name_key}
                    </td>

import { getProductDisplayNamePlain, IS_MC_PRODUCT } from '@/lib/product-branding';
...
export function ServiceDisclaimerIntro() {
  const name = getProductDisplayNamePlain();
  const muted = IS_MC_PRODUCT ? 'text-gray-600' : 'text-gray-400';
  return (
    <p className={muted}>
      現時点で、運営（{name}を運営する者。以下「運営」といいます）の責任について、次のとおり定めます。
    </p>
  );
}
...
export function ServiceDisclaimerList() {
  ...
  return (
    <ul className={`list-disc space-y-2 pl-5 ${muted}`}>
      <li>
        本サービスは<strong className={strong}>現状有姿</strong>
        で提供します。正確性・完全性・有用性・特定目的への適合、不具合・中断・遅延が生じないこと、他者による迷惑・違法行為が行われないことについて、運営は
        <strong className={strong}>いかなる保証も行いません</strong>。
      </li>
      <li>
        <strong className={strong}>YouTube 等の第三者のサービス・コンテンツ</strong>、
        <strong className={strong}>他の利用者の発言・行為</strong>
        {!IS_MC_PRODUCT ? (
          <>
            、<strong className={strong}>AI の出力</strong>
          </>
        ) : null}
        、
        ...
        <strong className={strong}>運営に故意または重過失がある場合を除き</strong>、運営は責任を負いません。利用者同士の紛争は当事者間で解決してください。
      </li>
      <li>
        上記のほか、本サービスの<strong className={strong}>利用または利用不能</strong>
        に関連して利用者に生じた損害（逸失利益、データの消失・改ざん、間接損害、精神的損害、付随的損害等を含みます）についても、
        <strong className={strong}>運営に故意または重過失がある場合を除き</strong>、運営は賠償義務を負いません。
      </li>
      <li>
        消費者契約法その他の法令により前各号の全部または一部が無効とされる場合、またはその他の理由で運営に損害賠償その他の責任が認められる場合においても、運営が負う義務の内容・範囲は
        <strong className={strong}>法令で認められる範囲</strong>に限ります。
      </li>
      <li>

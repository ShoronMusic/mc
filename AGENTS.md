...
| 曲解説パック API | `src/app/api/ai/comment-pack/route.ts` | 基本1本＋自由3本。上記ポリシーを利用。クライアントは `recentMessages`（直近 user/ai）を送れる。`COMMENT_PACK_SESSION_CONTEXT=0` で会話文脈注入オフ。開発で基本1本のみ＋選曲直後の announce 非表示は `.env.local` に `NEXT_PUBLIC_DEV_MINIMAL_SONG_AI=1`。**曲解説後クイズ**: `SONG_QUIZ_AFTER_COMMENTARY_ENABLED=1` で comment-pack／commentary の JSON に `songQuiz`（`includeQuiz`＝公式ヒューリスティック `allow` 時のみ true）。追加公式 `channelId` は `SONG_QUIZ_OFFICIAL_CHANNEL_IDS`。**三択本文**は `POST /api/ai/song-quiz`（`commentaryContext` に曲解説連結テキスト）。ゲート: `song-quiz-after-commentary`・`song-quiz-official-heuristic`、生成: `song-quiz-generate.ts`。**正解・正解者の発表**（同期部屋）: 回答は Ably `song-quiz:answer`、最古入室者が `NEXT_PUBLIC_SONG_QUIZ_REVEAL_MS`（既定 90000、最小 5000）後に AI 行で `【クイズ結果】…` を1本配信。`src/lib/song-quiz-result-announcement.ts` |

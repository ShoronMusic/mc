...
画面の例は `public/images/first-time-song-selection/` にも同じ画像を置いています（元データはリポジトリ直下の `_work`）。
...
![YouTube の動画ページとアドレスバー](../public/images/first-time-song-selection/youtube-01-video-page.png)

1. アドレスバーの URL をクリックして選択する（またはドラッグで選択）。
2. 右クリックして **「コピー」** を選ぶ。  
   キーボードなら **Ctrl+C**（Mac は **⌘+C**）でもコピーできます。

![アドレスバーで右クリックし「コピー」を選ぶ例](../public/images/first-time-song-selection/youtube-02-address-bar-copy.png)

アドレスバー全体の見え方の参考です。

![アドレスバーに表示された動画 URL の例](../public/images/first-time-song-selection/youtube-03-address-bar-full.png)

### 方法 B：「共有」からコピー

動画の下にある **「共有」** をクリックし、表示された窓の中のリンク横の **「コピー」**（青いボタン）を押します。`youtu.be/...` の短い形式でコピーされますが、そのまま MusiAi で使えます。

![共有ダイアログから「コピー」する例](../public/images/first-time-song-selection/youtube-04-share-copy.png)
...
![URL を貼り付けて「送信」する直前の例](../public/images/first-time-song-selection/musicai-01-paste-url-send.png)
...
![再生中とチャットに選曲案内が出た例](../public/images/first-time-song-selection/musicai-02-playing-announce.png)

![視聴履歴テーブルに記録された例](../public/images/first-time-song-selection/musicai-03-history-table.png)

...
| `pwa-phase2-youtube-share-receive.md` | Phase 2: YouTube 共有 → PWA 受け取り |
...
### Phase 2 — 共有・OAuth 実機（着手）

| ID | 項目 | 状態 | メモ |
|----|------|------|------|
| P2a | YouTube 共有 → `/share` | 済 | `pwa-phase2-youtube-share-receive.md` |
| P2b | マイリスト入口 | 未 | `my-list-spec.md` |
| P2c | OAuth standalone 実機 | 未 | 本番 HTTPS |

共有 URL 受け取りの詳細は `pwa-phase2-youtube-share-receive.md`。計画全体は `pwa-mobile-app-plan.md` §5。
...
| `src/app/share/page.tsx` | Web Share Target 受け口 |
| `src/lib/youtube-canonical-watch-url.ts` | 共有 URL の正規化 |
| `src/lib/share-target-pending.ts` | 保留 URL・直近部屋 |

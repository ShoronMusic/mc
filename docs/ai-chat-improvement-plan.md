...
| C3 | 同一曲の「版違い（Remastered / Official Video など）」を、表示名と集計キーで分離管理する（表示は版情報付き、集計は正規化タイトル＋メインアーティストで寄せる） |
| C4 | 分離禁止アーティスト（`artist-compound-extra.json`）と Music8 スラッグ別名（`music8-main-artist-aliases.json`）を同時運用し、`Echo & The Bunnymen` などの表記ゆれでも同一アーティストJSONへ確実に到達させる |
...
## 2026-04-08 追記メモ（今後課題）

- YouTube の別動画・別表記（例: 通常版 / Remastered / Official ラベル差）で同一楽曲が分離されるため、**保存時の正規化キー設計**を検討する。  
  - 案: `display_title`（版付き表示）と `canonical_title`（版ノイズ除去）を分離し、集計・レコメンドは `canonical_*` で統合。
- `mylist_oembed` 経路は古いMVでのアップローダー名混入（例: RHINO）や括弧付き曲名（例: `Pride (In The Name Of Love)`）に影響されやすいため、**回帰テストセットを拡充**する。
- 例外管理は「増え続ける前提」で、JSONマスタ更新手順（追加条件・テスト必須・キャッシュ再生成手順）を運用手順として文書化する。

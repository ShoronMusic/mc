...
- 視聴履歴はルーム単位で DB に継続保存する（会ごとに DELETE しない）。
- 部屋内 UI の視聴履歴・年代／スタイル分布・チャットサマリーは、**開催中の会**では `room_gatherings.started_at` 以降に限定して表示する（別主催者の過去分を見せない）。
...
- 視聴履歴: `room_playback_history`。DB は room 単位で継続記録。部屋 UI は開催中会は当該 gathering の `started_at` 以降のみ表示。

...
### 2.4 「ホーム画面に追加」バナーが出ないとき

下部の **「ホーム画面に追加すると便利です」** は、次のとき **出ません**（仕様）。

| 理由 | 対処 |
|------|------|
| 以前 **「閉じる」** を押した | **14 日間**非表示。トップの **「ホーム画面に追加（アプリとして使う）」** リンクをタップするか、`https://musicai.jp/?pwa_install=1` を開く |
| すでに **ホーム画面のアイコン**から起動している | 追加済み（standalone）。バナー不要 |
| **PC** またはスマホで **デスクトップ版サイト**を要求している | スマホの Chrome で開き直す。メニューから **アプリをインストール** / **ホーム画面に追加** |

**手動で追加（Android Chrome）**: 右上 **⋮** → **アプリをインストール** または **ホーム画面に追加**  
**iPhone Safari**: **共有**（□↑）→ **ホーム画面に追加**

### 2.5 UI が真っ白・ボタンだけ巨大（スタイル未適用）のとき
...
### 2.6 `missing required error components, refreshing...` のとき
...
### 2.7 YouTube 共有に「洋楽AIチャット」が二つ出る

**サーバー側の manifest は1つだけ**です。共有先が2つあるのは、Android に **同じ PWA が二重登録**されていることがほとんどです（再インストールの残り・「ホーム画面に追加」と「アプリをインストール」の両方・本番とプレビュー URL を別々に追加した、など）。

**症状（二重登録時）**

- 共有シートのアイコン下に **「洋楽AIチャ…」が2行**（1アイコンに2つ分のラベル、またはアイコンが2つ）
- **2回目の共有**で部屋から追い出され、トップに「共有などで同じ部屋がもう一度開かれたため…」と出る
- トップでは **小龍としてログイン中**のまま（再ログインは不要だが、部屋だけ抜けている）

→ 共有のたびに `/share` → 部屋の**全リロード**が走り、同じアカウントの「端末取り合い」になっていた（コード側も修正済み: 部屋が開いていれば URL だけ渡す）。

**1つにまとめる手順（Android）**

1. ホーム画面の **洋楽AIチャット** アイコンを **すべて削除**
2. **設定 → アプリ** で「洋楽AIチャット」等を検索し、**同系が2つあれば両方アンインストール**
3. Chrome で **https://musicai.jp** を開き、**一度だけ**「アプリをインストール」または「ホーム画面に追加」
4. YouTube 共有で **1つ**になったか確認

どちらをタップしても `/share` には届きますが、**1つに整理**しないと再入室・端末選択の混乱が続きます。
...
| 9 | YouTube 共有（Android PWA） | YouTube → 共有 → 洋楽AIチャット → **直近の部屋**で発言欄に `watch?v=` が入る。**再ログインなし**で部屋に戻れること（数秒「読み込み中」は可） |
| 10 | PWA 他アプリ復帰 | 部屋入室中に YouTube 等へ切替 → MC に戻る → **JoinChoice を出さず**「部屋に戻っています…」のあとそのまま部屋（再入室 UI なし） |

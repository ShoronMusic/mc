...
**サーバー側の manifest は1つだけ**です。共有先が2つあるのは、Android に **同じ PWA が二重登録**されていることがほとんどです（再インストールの残り・「ホーム画面に追加」と「アプリをインストール」の両方など）。
...
どちらをタップしても `/share` には届きますが、**1つに整理**した方が再ログインや部屋の記憶が安定しやすいです。

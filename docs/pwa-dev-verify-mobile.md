...
- 共有シートに **同じ名前が2つ**
- 片方をタップすると **「同じアカウントが別の端末で参加中です」**（実際は1台だけ）と出る
- 部屋に入った直後に **トップへ戻される**・**再入室**を求められる

→ 2つの PWA が **別プロセス**として動き、同じアカウントの「端末取り合い」になっている状態です。**MC を1つしか起動していなくても**、OS 上は2つ登録されているとこうなります。
